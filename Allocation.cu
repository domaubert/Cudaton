...
#ifdef WMPI
  buff=(float*) calloc(NGRP*NBUFF*NBUFF*4,sizeof(float)); // 4 for EGY + FLUX // HERE WE ASSUME THAT NCELLZ >=NCELLX,Y
#endif
}
...
#ifdef WMPI
  cudaMalloc((void**)&cubuff,NGRP*NBUFF*NBUFF*4*sizeof(float)); // 4 for EGY + FLUX // HERE WE ASSUME THAT NCELLZ >=NCELLX,Y
#endif
