...
//**********************************************************************************
__global__ void cuComputeELF(float *cuegy, float *cuflx, float *cusrc0, float *cuegy_new, float c, float dx, float dt, int iter, float aexp)
{
  int tx=threadIdx.x+NBOUND;
  int bx=blockIdx.x +NBOUND;
  int by=blockIdx.y +NBOUND;

  float um1,up1,fm1,fp1,u0;
  ...
  res=u0-0.5*((fp1-fm1)+c*(2*u0-um1-up1))/dx*dt; // 10 FLOP
  ...
  res=res-0.5*((fp1-fm1)+c*(2*u0-um1-up1))/dx*dt; // 10 FLOP
  ...
  res=res-0.5*((f[tx+1]-f[tx-1])+c*(2*u[tx]-u[tx+1]-u[tx-1]))/dx*dt; // 10 FLOP

#ifndef SDISCRETE
  float s=cusrc0[baseidu+tx]/dx/dx/dx;
  cuegy_new[baseidu+tx]=res+s*dt;
#else
  cuegy_new[baseidu+tx]=res;
#endif
}
...
//**********************************************************************************
__global__ void cuComputeF_TOTAL_LF(float *cuflx, float *cudedd, float *cusrc0, float *cuflx_new, float c, float dx, float dt, int iter, float *cuegy, float aexp)
{
  int tx=threadIdx.x+NBOUND;
  int bx=blockIdx.x +NBOUND;
  int by=blockIdx.y +NBOUND;
...
#ifndef TESTCOOL // C'est celui la
__global__ void cuComputeTemp(float *cuxion, float *cudensity, float *cutemperature, float *cuegy_new, float fudgecool, float c, float dt,float unit_number, int ncvgcool, float aexp, float hubblet, float *cuflx_new, float CLUMPF)
{
  int 	tx=threadIdx.x,
	bx=blockIdx.x,
	by=blockIdx.y,
...
#ifndef MULTILO
  float	hnu=AVG_EGY*1.6022e-19,	// ! Average Photon Energy (J)
	alphae=AVG_CSE*c, 	// switch back to physical velocity
	alphai=AVG_CSN*c,
	et, 
	p;
#else
...
//  float CLUMPI==fmaxf(fminf(powf(nH[idloc]/400.)^2.5+powf(nH[idloc]/8.)^0.2*(1-x0[idloc]),100.),1.); 
  ...
  while(currentcool_t<dt)
    {
      nitcool++;
      eint=1.5*nH[idloc]*KBOLTZ*(1.+x0[idloc])*tloc[idloc];
...
#ifndef MULTILO
      et=((alpha-alphab)*x0[idloc]*x0[idloc]*nH[idloc]*nH[idloc]*dtcool+egyloc[idloc])/(1.f+dtcool*(alphai*(1.f-x0[idloc])*nH[idloc]+3*hubblet));

      if(et<0) 
	{fudgecool/=10.f;
	  continue;}
      p=(1.f+(alphai*nH[idloc]*(1-x0[idloc])+2*hubblet)*dtcool);
#else
  bool test = 0;
  for(igrp=0;igrp<NGRP;igrp++)
	{
	ai_tmp1 = alphai[igrp];
	et[igrp]=((alpha-alphab)*x0[idloc]*x0[idloc]*nH[idloc]*nH[idloc]*dtcool+egyloc[idloc+igrp*BLOCKCOOL])/(1.f+dtcool*(ai_tmp1*(1.f-x0[idloc])*nH[idloc]+3*hubblet))* (igrp==0);
...
#ifndef MULTILO
	ai_tmp1 = (alphae*hnu-alphai*hnu0)*et;
#else
...
for(igrp=0;igrp<NGRP;igrp++)
	{
  cuegy_new[idx+igrp*NCELL4]=egyloc[idloc+igrp*BLOCKCOOL]*aexp*aexp*aexp;
...
#else
__global__ void cuComputeTemp(float *cuxion, float *cudensity, float *cutemperature, float *cuegy_new, float fudgecool, float c, float dt,float unit_number, int ncvgcool, float aexp, float hubblet,float *cuflx, float CLUMP)
{
  float hnu;
