...
void getalist(void)
{
  ...
  for(i=0;i<nalist;i++)
	...
	tlist[i]=0.;
      ...
      printf("(%f %f) ",alist[i],tlist[i]/unit_time);
    }
  //  printf("\n");
  //  printf("\n");
}
...
void cuDumpResults(int i, float time, float aexp, int rank)
{
  ...
  cudaMemcpy(temperature,cutemperature,NCELLS3*sizeof(float),cudaMemcpyDeviceToHost);
   printf("src=%e\n",src0[(62+NBOUND)+(43+NBOUND)*(NCELLX+2*NBOUND)+(16+NBOUND)*(NCELLX+2*NBOUND)*(NCELLY+2*NBOUND)]);
  fp=fopen(fname,"wb");
...
void cuGetIC(int iic,int rank)
{
  ...
  printf("nc=%d ns=%d t=%e\n",nc,ns,t);
  fread(egy,sizeof(float),NGRP*NCELLS3,fp);
  fread(flx,sizeof(float),NGRP*3*NCELLS3,fp);
...
fclose(src);
  ...
  for(i=0;i<NCELLS3;i++) density[i]=fmaxf(egy_min,density[i]);
